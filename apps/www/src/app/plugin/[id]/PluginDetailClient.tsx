...
export default function PluginDetailClient({ pluginData, isSteamClient, apiError, mdOverrides }: PluginDetailClientProps) {
	if (apiError) return <RenderAPIError />;

	function copyPluginId() {
		navigator.clipboard
			.writeText(pluginData?.id)
			.then(() => toast.success('Successfully copied to clipboard!'))
			.catch(() => toast.error('Failed to copy plugin ID'));
	}
											...
											<div className="addon-details-segment" id="addon-details-column-actions">
												...
												<section id="addon-actions">
													<div className="btn-container direction-column">
														...
														<Tooltip id="verified-plugin-tooltip" place="bottom-start" className="tooltip">
												...
												<section id="about-addon">
													...
													<span className="addon-metadata-row">
														...
														<a rel="noreferrer noopener" target="_blank" className="anchor" href={`https://github.com/${pluginData?.repoOwner}/${pluginData?.repoName}/tree/${pluginData?.commitId}`}>
															{`https://github.com/${pluginData?.repoOwner}/${pluginData?.repoName}/tree/${pluginData?.commitId}`}
														</a>
													</span>
												</section>
											</div>
									...
									<div className="addon-details-column" id="addon-details-left-column">
										<article className="addon-details-segment markdown-readme-content">
											<div className="markdown-body">
												<div dangerouslySetInnerHTML={{ __html: pluginData?.readMeMarkdown }} suppressHydrationWarning />
											</div>
										</article>
									</div>
								</div>
